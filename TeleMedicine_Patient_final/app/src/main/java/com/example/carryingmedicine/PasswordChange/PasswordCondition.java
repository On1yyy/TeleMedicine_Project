...
public class PasswordCondition
{
    ...
    public PasswordCondition(PasswordChangeActivity inActivity)
    { //생성자 이렇게선언하기 toast 쓸려면
        changeActivity = inActivity;
    }

   public boolean user_id_check(String user_id){
       Matcher id_matcher = idPattern.matcher(user_id);
       if(!id_matcher.matches()){
           Toast.makeText(changeActivity.getApplicationContext(), "아이디는 영문, 숫자로 입력 가능하고 6에서 12자리로 입력해주세요", Toast.LENGTH_SHORT).show();
           return false;
       }
       else
           {
    ...
    public boolean user_name_check(String user_name)
    {
        ...
        if(!n_matcher.matches()){
            Toast.makeText(changeActivity.getApplicationContext(), "이름은 한글로만 입력하세요", Toast.LENGTH_SHORT).show();
            return false;
        }
        else
        {
    ...
    public boolean user_pass_check(String user_pass, String user_id)
    {
        ...
        if(!matcher.matches()){
            Toast.makeText(changeActivity.getApplicationContext(), "비밀번호는 숫자 문자 특수문자 포함 8~15자가 되어야합니다.", Toast.LENGTH_SHORT).show();
            return false;
        }

        else if(user_pass.indexOf(user_id)>-1){
            Toast.makeText(changeActivity.getApplicationContext(), "비밀번호에 아이디형식이 들어갈 수 없습니다.", Toast.LENGTH_SHORT).show();
            return false;
        }
        else if(user_pass.contains(" ")) {
            Toast.makeText(changeActivity.getApplicationContext(), "비밀번호엔 공백을 포함할수 없습니다.", Toast.LENGTH_SHORT).show();
            return false;
        }
        else
            return true;
